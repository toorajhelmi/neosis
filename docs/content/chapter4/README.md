...
We consider an $$m$$-bit NeoVerse projection $$U_t = (U_t(1), \ldots, U_t(m)) \in \{0,1\}^m$$, where each bit evolves as an independent binary Markov chain with flip probability $$\alpha \in [0,1]$$, so that $$P(U_{t+1}(j) \neq U_t(j)) = \alpha$$ and $$P(U_{t+1}(j) = U_t(j)) = 1 - \alpha$$. Each coordinate is a symmetric two-state Markov chain with transition matrix $$P = \begin{pmatrix} 1-\alpha & \alpha \\ \alpha & 1-\alpha \end{pmatrix}$$. The stationary distribution $$\pi = (\pi(0), \pi(1))$$ satisfies $$\pi P = \pi$$, which after substituting $$\pi(1) = 1 - \pi(0)$$ into $$\pi(0) = \pi(0)(1-\alpha) + \pi(1)\alpha$$ yields $$2\alpha \pi(0) = \alpha$$, and therefore $$\pi(0) = \pi(1) = \frac{1}{2}$$. Because all $$m$$ bits evolve independently, the joint stationary distribution for the $$m$$-bit vector is the uniform product measure $$\pi_{\text{NV}}(x) = 2^{-m}$$ for all $$x \in \{0,1\}^m$$, which is the Stationary Distribution (SD) of the NeoVerse that the Neo experiences at its input.

The Neo motif we study is a **copy Neo**, which reads the NV projection perfectly and outputs it directly as $$Y_t = U_t$$. As a predictor, it uses the simple rule $$\hat{u}(Y_t) = Y_t$$, predicting that each bit at the next step will equal its current value. For this Markov NV, this is the optimal predictor as long as $$\alpha \leq \frac{1}{2}$$, since each bit is more likely to stay the same than to flip.

#### 4.3.2 Exact-Vector Reward: Sparks Only If All Bits Are Correct

We first assume that the Neo receives a Spark only when it predicts the entire $$m$$-bit vector correctly. Defining $$U^+ = U_{t+1}$$ and accuracy as $$\text{Acc} = P(\hat{u}(Y_t) = U^+)$$, we note that for a single bit the probability of being correct is $$P(U_{t+1}(j) = \hat{u}^{(j)}(Y_t)) = P(U_{t+1}(j) = U_t(j)) = 1 - \alpha$$. Because bits are independent, the probability that all $$m$$ bits are predicted correctly is $$p := \text{Acc} = (1-\alpha)^m$$.

Letting $$r > 0$$ denote the Nex gained when the Neo receives a Spark and $$c_\ell > 0$$ the per-tick living cost, the per-tick Nex change is $$\Delta E_t = r \, \mathbb{1}\{\hat{u}(Y_t) = U^+\} - c_\ell$$, which in the stationary regime takes the value $$r - c_\ell$$ with probability $$p$$ and $$-c_\ell$$ with probability $$1 - p$$. The mean drift is $$\mu = \mathbb{E}[\Delta E_t] = r p - c_\ell = r(1-\alpha)^m - c_\ell$$, and the variance is $$\sigma^2 = \text{Var}(\Delta E_t) = r^2 p(1-p) = r^2 (1-\alpha)^m [1 - (1-\alpha)^m]$$.

Letting $$E_t$$ denote the Neo's Nex at tick $$t$$, starting from $$E_0 > 0$$ and evolving as $$E_{t+1} = E_t + \Delta E_t$$ with absorption at $$E_t = 0$$, we approximate this biased random walk by a diffusion with drift $$\mu$$ and variance $$\sigma^2$$. The survivability $$\Xi$$ then satisfies $$\Xi \approx 0$$ if $$\mu \leq 0$$, and $$\Xi \approx 1 - \exp(-2\mu E_0 / \sigma^2)$$ if $$\mu > 0$$. Substituting $$\mu$$ and $$\sigma^2$$ yields
...
In this regime, effective accuracy decays as $$(1-\alpha)^m$$, so even modest noise levels $$\alpha$$ become catastrophic as the prediction dimension $$m$$ grows, causing the Neo to quickly fall below the survival threshold unless rewards or costs are adjusted.

#### 4.3.3 Proportional Reward: Sparks Per Correct Bit

We now consider a more graded reward scheme where the Neo receives Sparks proportional to the fraction of correctly predicted bits. Letting $$K_t$$ denote the number of correctly predicted bits at tick $$t$$, and given the same copy Neo and NV dynamics where each bit is correct with probability $$q = 1 - \alpha$$ independently across bits, we have $$K_t \sim \text{Binomial}(m, q)$$. If a fully correct prediction yields reward $$r$$, then a fractionally correct prediction yields $$R_t = r \cdot K_t / m$$, and the Nex change per tick is $$\Delta E_t = R_t - c_\ell = r K_t / m - c_\ell$$.

The mean drift is $$\mu = \mathbb{E}[\Delta E_t] = r \mathbb{E}[K_t] / m - c_\ell = r mq / m - c_\ell = r(1-\alpha) - c_\ell$$, and the variance is $$\sigma^2 = \text{Var}(\Delta E_t) = (r/m)^2 \text{Var}(K_t) = (r/m)^2 mq(1-q) = r^2 (1-\alpha)\alpha / m$$. Again approximating $$E_t$$ by a diffusion with drift $$\mu$$ and variance $$\sigma^2$$, survivability satisfies $$\Xi \approx 0$$ if $$\mu \leq 0$$, and $$\Xi \approx 1 - \exp(-2\mu E_0 / \sigma^2)$$ if $$\mu > 0$$. Substituting $$\mu$$ and $$\sigma^2$$ gives
